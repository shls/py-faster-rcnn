...
class SVMTrainer(object):
    ...
    def _get_feature_scale(self, num_images=100):
        ...
        count = 0.0
        inds = npr.choice(
            xrange(self.imdb.num_images), size=num_images,
            replace=False
        )
    ...
    def _get_pos_counts(self):
        ...
        for j in xrange(1, self.imdb.num_classes):
            print('class {:s} has {:d} positives'.format(
                self.imdb.classes[j], counts[j]
            ))
    ...
    def get_pos_examples(self):
        ...
        for i in xrange(num_images):
            ...
            print('get_pos_examples: {:d}/{:d} {:.3f}s'.format(
                i + 1, len(roidb), _t.average_time
            ))
    ...
    def train_with_hard_negatives(self):
        ...
        for i in xrange(num_images):
            ...
            for j in xrange(1, self.imdb.num_classes):
                hard_inds = np.where(
                    (scores[:, j] > self.hard_thresh) &
                    (roidb[i]['gt_overlaps'][:, j].toarray().ravel() < self.neg_iou_thresh)
                )[0]

                if len(hard_inds) > 0:
                    hard_feat = feat[hard_inds, :].copy()
                    new_w_b = self.trainers[j].append_neg_and_retrain(
                        feat=hard_feat
                    )
            ...
            print('train_with_hard_negatives: {:d}/{:d} {:.3f}s'.format(
                i + 1, len(roidb), _t.average_time
            ))
...
class SVMClassTrainer(object):
    ...
    def train(self):
        ...
        print('Cache holds {} pos examples and {} neg examples'.format(
            num_pos, num_neg
        ))

        X = np.vstack((self.pos, self.neg)) * self.feature_scale
        y = np.hstack(
            (np.ones(num_pos), -np.ones(num_neg))
        )
