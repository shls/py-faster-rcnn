...
params_m = ['conv1']
params_s = ['conv2','conv3','conv4','conv5', 'rpn_conv/3x3', 'rpn_cls_score', 'rpn_bbox_pred', 'roi_pool5', 'fc6','fc7','cls_score', 'bbox_pred']
...
# Load New net
new_net = caffe.Net('/home/ls/event_recognition/net_surgery/rcnn/sdha.prototxt', caffe.TRAIN)
new_params_m = ['conv1']
...
for pr, new_pr in zip(params_m, new_params_m):
	for n in range (0, fc_params_m[pr][0].shape[0]):
		for c_i in range (0, fc_params_m[pr][0].shape[1]):
			new_fc_params_m[new_pr][0][n][0] += fc_params_m[pr][0][n][c_i]
		new_fc_params_m[new_pr][0][n][0] /= fc_params_m[pr][0].shape[1]
		for new_c_i in range(1, new_channels):
			new_fc_params_m[new_pr][0][n][new_c_i] = new_fc_params_m[new_pr][0][n][0]
...
new_net.save('/home/ls/net_surgery/new_VGG_CNN_S.caffemodel')
